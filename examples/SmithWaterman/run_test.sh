...
make run WORKLOAD_ARGS="inputs/string1.txt inputs/string2.txt 3 3" 2>&1 | tee $TMPFILE
fgrep -q 'score: 10' < $TMPFILE  && echo OK || echo FAIL

echo "Testing SA.txt vs SB.txt with width=569, height=661"
make run WORKLOAD_ARGS="inputs/SA.txt inputs/SB.txt 569 1983" 2>&1 | tee $TMPFILE
