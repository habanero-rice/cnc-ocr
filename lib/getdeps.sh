...
if ! [ -f .depsOK ]; then
    ...
    $CURL https://pypi.python.org/packages/source/M/MarkupSafe/MarkupSafe-0.23.tar.gz \
        | tar xz MarkupSafe-0.23/markupsafe --strip=1 && \
    $CURL https://pypi.python.org/packages/source/J/Jinja2/Jinja2-2.7.3.tar.gz \
        | tar xz Jinja2-2.7.3/jinja2 --strip=1 && \
    $CURL https://pypi.python.org/packages/source/a/argparse/argparse-1.2.1.tar.gz \
        | tar xz argparse-1.2.1/argparse.py --strip=1 && \
    touch ../.depsOK || exit 1
fi
